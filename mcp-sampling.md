...
3.  The database tool returns raw metrics to goose:
    ...
    ```
    Query times: 2.3s, 1.8s, 5.2s, 0.3s, 8.1sTable sizes: users (1M rows), orders (5M rows)Indexes: 3 on users, 1 on orders
    ```
...
Want to add MCP Sampling to your own extensions? See our [Building Custom Extensions](/goose/docs/tutorials/custom-extensions) tutorial to learn more about how MCP servers can leverage goose's AI capabilities.
