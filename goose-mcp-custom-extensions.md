...
You can checkout other examples in this [MCP servers repository](https://github.com/modelcontextprotocol/servers). MCP SDKs are also available in [Typescript](https://github.com/modelcontextprotocol/typescript-sdk) and [Kotlin](https://github.com/modelcontextprotocol/kotlin-sdk).

info

goose currently supports Tools and Resources for [MCP Server features](https://spec.modelcontextprotocol.io/specification/2024-11-05/server/). We will be adding support for MCP Prompts soon.
...
```
uv init --lib mcp-wikicd mcp-wikimkdir -p src/mcp_wikitouch src/mcp_wiki/server.pytouch src/mcp_wiki/__main__.py
```

Your project directory structure should look like this:

```
.├── README.md├── pyproject.toml├── src│   └── mcp_wiki│       ├── __init__.py   # Primary CLI entry point│       ├── __main__.py   # To enable running as a Python module│       ├── py.typed      # Indicates the package supports type hints│       └── server.py     # Your MCP server code (tool, resources, prompts)└── uv.lock
```
...
5.  In the UI, you can click "Connect" to initialize your MCP server. Then click on "Tools" tab > "List Tools" and you should see the `read_wikipedia_article` tool. Then you can try to call the `read_wikipedia_article` tool with URL set to "[https://en.wikipedia.org/wiki/Bangladesh](https://en.wikipedia.org/wiki/Bangladesh)" and click "Run Tool".
...
**[MCP Sampling](/goose/docs/guides/mcp-sampling)** allows your MCP servers to request AI completions from goose's LLM, transforming simple tools into intelligent agents.
...
**[MCP-UI Extensions](/goose/docs/guides/interactive-chat/mcp-ui)** enable rich, interactive user interfaces instead of text-only responses, transforming static MCP servers into dynamic, engaging experiences.
